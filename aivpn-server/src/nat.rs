...
impl NatForwarder {
    ...
    fn setup_iptables(&self) -> Result<()> {
        ...
        ipt_ensure(
            "nat",
            "POSTROUTING",
            &["-s", &cidr, "-m", "comment", "--comment", c, "-j", "MASQUERADE"],
        );
        ipt_ensure(
            "filter",
            "FORWARD",
        ...
        ipt_ensure(
            ...
            "FORWARD",
            &[
                "-o", tun, "-m", "state", "--state", "RELATED,ESTABLISHED",
        ...
        ipt_ensure(
            ...
            "FORWARD",
            &[
                "-o", tun, "-p", "tcp", "--tcp-flags", "SYN,RST", "SYN",
                "-m", "comment", "--comment", c, "-j", "TCPMSS", "--clamp-mss-to-pmtu",
            ],
        );
        ipt_ensure(
            "mangle",
            "FORWARD",
            &[
                "-i", tun, "-p", "tcp", "--tcp-flags", "SYN,RST", "SYN",
                "-m", "comment", "--comment", c, "-j", "TCPMSS", "--clamp-mss-to-pmtu",
            ],
        );
