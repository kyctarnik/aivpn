...
Маршрутизация трафика клиента через два узла AIVPN без изменений на стороне клиента. Входной узел пересылает зашифрованные IP-пакеты на выходной узел, который выходит в интернет. Для клиента ничего не меняется — он общается только с входным узлом. Интернет видит IP выходного узла.
...
**Входной узел** (`server.json`):
```json
{
  "pool": {
    "sync_key": "<общий-base64-ключ-32-байта>",
    "exit_node": "exit.example.com:443"
  }
}
```

**Выходной узел** (`server.json`):
```json
{
  "pool": {
    "sync_key": "<тот-же-ключ>",
    "exit_node_enabled": true
  }
}
```

> Оба узла должны использовать одинаковый `sync_key`. Сгенерировать: `openssl rand -base64 32`
> На выходном узле необходимо явно указать `exit_node_enabled: true` — по умолчанию `false`, чтобы исключить использование в качестве открытого прокси.

### DNS-over-HTTPS прокси

Блокирует утечки DNS и направляет весь DNS-трафик клиентов через зашифрованный DoH-резолвер на VPN-интерфейсе.
...
Сборка: `cargo build --release --bin aivpn-server --features "dns"`

`block_plain_dns: true` добавляет правило nftables, блокирующее UDP/53 на всех интерфейсах кроме TUN, чтобы клиенты не могли обойти прокси, даже вручную задав DNS-сервер.

### Сеть сайт-сайт (Site-to-site VPN)

Объединяет два и более узла AIVPN так, чтобы хосты разных площадок видели друг друга без клиентского ПО. Пиры обмениваются маршрутами через тот же крипто-стек, что и пул-синхронизация.
...
Маршруты устанавливаются через `ip route add` при получении объявления от пира (каждые 30 с). Принимаются только подсети из allowlist `remote_subnets` — произвольное внедрение маршрутов заблокировано.

### Лёгкий mTLS (клиентские сертификаты)

Необязательный второй фактор аутентификации: 104-байтовый ed25519-сертификат поверх существующего X25519 + PSK. Клиенты без сертификатов продолжают работать при `required: false` (по умолчанию).
...
- `required: false` — клиенты без сертификата принимаются; сертификат проверяется при наличии
- `required: true` — без корректного сертификата отправка данных заблокирована

Формат: `client_pub_key[32] || expiry_ts_le[8] || ca_signature[64]` (104 байта, без новых зависимостей — используется существующий `ed25519-dalek`).

### Телеметрия дропов eBPF XDP

XDP-фильтр ведёт счётчики дропов по причинам (`TOO_SHORT`, `TAG_EXPIRED`, `TOTAL`) через кольцевой BPF-буфер. `ebpf_observer.rs` читает их через сырые BPF syscall и публикует дельта-события `XdpDrop` в `EventBus`. Активируется автоматически при наличии `/sys/fs/bpf/aivpn/drop_stats`.
