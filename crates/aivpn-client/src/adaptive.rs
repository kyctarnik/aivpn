...
impl AdaptiveMonitor {
    ...
    fn maybe_adapt(&self) {
        ...
        let loss = g.loss_pct();
        // NOTE (B2): the MTU step-down that used to live here was removed —
        // it was never wired to real loss detection (no caller of
        // `on_packet_loss` exists in the data plane) and the TUN MTU is fixed
        // at creation time anyway. Loss resilience comes from AdaptiveLevel's
        // score-driven keepalive/FEC path (quality.rs + client.rs) instead.
        if loss >= self.config.loss_threshold_pct {
            g.mode = AdaptiveMode::Adapting;
            info!("Adaptive: loss={:.1}% → adapting (faster keepalive)", loss);
        } else if loss < self.config.loss_threshold_pct / 2.0 && g.mode != AdaptiveMode::Normal {
            g.mode = AdaptiveMode::Normal;
            info!("Adaptive: loss recovered — back to normal");
        }
    }
    ...
    pub fn keepalive_factor(&self) -> f32 {
        ...
        if self.inner.lock().unwrap_or_else(|e| e.into_inner()).mode != AdaptiveMode::Normal {
